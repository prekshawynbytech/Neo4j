...
AVAILABLE_RELATIONSHIPS = [
    Schema("Student", "HAS_SCORE", "Subject")
]
...
class Entities(BaseModel):
    ...
    name: List[str] = Field(
        ...,
        description="All the students, subjects, or scores that appear in the text",
    )
...
class CustomCypherChain(GraphCypherQAChain):
    def process_entities(self, text: str) -> List[str]:
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are extracting subject and student entities from the text.",
                ),
                (
                    "human",
    ...
    def get_viz_data(self, entities: List[str]) -> List[Tuple[str, str]]:
        ...
        RETURN s.name AS source, type(r) AS type, sub.name AS target
        LIMIT 5
        UNION
        MATCH (s:Student)<-[r:HAS_SCORE]-(sub:Subject)
        WHERE s.name IN $entities
        RETURN s.name AS target, type(r) AS type, sub.name AS source
        LIMIT 5
        """
    ...
    def generate_system_message(
        ...
        if relevant_entities:
            system_message += (
                f"Entity Substitution: If the question mentions specific students, replace them in the query with corresponding students from "
                f"the given list. Given list of students is: {relevant_entities}\n"
                "Example: If the list contains Alice: ['Alice Smith', 'Alice Johnson'], replace 'Alice' in the query with 'Alice Smith' or 'Alice Johnson'.\n"
                "Flexible Queries: Ensure your Cypher queries can capture all relevant students.\n"
                "Correct: MATCH (s:Student) WHERE s.name IN ['Alice Smith', 'Bob Johnson'] MATCH (s)-[:HAS_SCORE]->(sub:Subject)"
            )
        if fewshot_examples:
            system_message += (
        ...
        system_message += (
            ...
            "Include details about both students and subjects, if applicable. \n"
